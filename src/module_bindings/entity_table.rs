...
/// which allows point queries on the field of the same name
/// via the [`EntityIdentityUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.entity().identity().find(...)`.
pub struct EntityIdentityUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Entity, __sdk::Identity>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> EntityTableHandle<'ctx> {
    /// Get a handle on the `identity` unique index on the table `entity`.
    pub fn identity(&self) -> EntityIdentityUnique<'ctx> {
        EntityIdentityUnique {
            imp: self
                .imp
...
impl<'ctx> EntityIdentityUnique<'ctx> {
